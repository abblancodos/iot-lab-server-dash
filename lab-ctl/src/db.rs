...
pub async fn init_schema(pool: &PgPool) -> anyhow::Result<()> {
    ...
    sqlx::query(r#"CREATE INDEX IF NOT EXISTS sync_audit_issue_type_idx ON sync_audit(issue_type)"#)
        .execute(pool).await?;

    sqlx::query(r#"CREATE INDEX IF NOT EXISTS sync_audit_resolved_idx ON sync_audit(resolved)"#)
        .execute(pool).await?;

    sqlx::query(r#"CREATE INDEX IF NOT EXISTS sync_audit_synced_at_idx ON sync_audit(synced_at DESC)"#)
        .execute(pool).await?;
